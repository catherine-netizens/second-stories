...
async function seedDatabase() {
  ...
  for (const name of sellerNames) {
    ...
    if (existing.length) {
      ...
      const [r] = await pool.query(
        `INSERT INTO users
        (name,email,password,location,bio,avatar)
        VALUES (?,?,?,?,?,?)`,
        [
          name,
          email,
  ...
  console.log(
    `Seed ${SS_PRODUCTS.length} produk demo berhasil.`
  );
}
    ...
    SELECT
      ...
      COALESCE(
        (
          ...
          WHERE rp.seller_id = p.seller_id
        ),
        0
      ) AS seller_rating
    ...
    try {
      ...
      if (
        ...
        return fail(
          res,
          400,
          `Jumlah melebihi stok yang tersedia. Maksimal ${p[0].stock}.`
        );
      }
    ...
    try {
      ...
      if (
        ...
        return fail(
          res,
          400,
          "Minimal satu foto barang diperlukan."
        );
      }
    ...
    try {
      ...
      if (
        ...
        return fail(
          res,
          400,
          "Kamu tidak bisa chat dengan akun sendiri."
        );
      }
  ...
  (err, req, res, next) => {
    ...
    if (
      ...
      return fail(
        res,
        413,
        "Ukuran total foto terlalu besar. Coba kurangi jumlah foto atau gunakan foto dengan ukuran file lebih kecil."
      );
    }
...
(async () => {
  ...
  try {
    ...
    console.error(
      "Pesan:",
      e.message ||
        "Tidak ada pesan error dari driver MySQL."
    );
